...
function BoardContent({
  ...
  useEffect(() => {
    // Column đã được sắp xếp ở component cha cao nhất (boards/_id.jsx) 71
  ...
  const handleDragEnd = (event) => {
    ...
    if (activeDragItemType === ACTIVE_DRAG_ITEM_TYPE.COLUMN) {
      ...
      if (active.id !== over.id) {
        ...
        // ME: kết quả trả ra là 1 mãng đã được thay đổi thứ tự
        // Code của arrayMove ở đây: dnd-kit/packages/sortable/src/utilities/arrayMove.ts
        const dndOrderedColumns = arrayMove(
          orderedColumns, // mãng columns (title, _id,.....)
          oldColumnIndex, // vị trí cũ của tk active
          newColumnIndex // vị trí mới của tk avtive
        )

        // Gọi update State ở đây để tránh delay hoặc Flickering giao diện lúc kéo thả cần phải chờ gọi API (small trick)
        setOrderedColumns(dndOrderedColumns)

        /**
         * - Gọi lên props function moveColumns nằm ở component cha cao nhất (boards/_id.jsx)
         * - Thì lúc này chúng ta có thể gọi luôn API ở đây là xong thay vì phải lần lược gọi ngược lên những component cha phía bên trên.
         */
